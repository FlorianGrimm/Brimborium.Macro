    ...
    where TNotification : INotification
{
    ...
    public readonly bool IsArray(
        [MaybeNullWhen(false)] out INotificationHandler<TNotification>[] handlers
    )
    {
        if (_isArray)
        {
    ...
    public readonly bool IsSingleHandler(
        [MaybeNullWhen(false)] out INotificationHandler<TNotification> handler
    )
    {
    ...
    IEnumerator<INotificationHandler<TNotification>> IEnumerable<
        INotificationHandler<TNotification>
    >.GetEnumerator() => new Enumerator(in this);
    ...
    public struct Enumerator : IEnumerator<INotificationHandler<TNotification>>
    {
        ...
        readonly object? IEnumerator.Current
        {
            get
            {
                if (
                    (
                        _handlers.IsArray(out var array)
                        && (_index == 0 || _index == array.Length + 1)
                    ) || _enumerator is null
                )
                {
